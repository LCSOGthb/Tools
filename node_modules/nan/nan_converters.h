...
template <typename T> struct ValueFactoryBase {
  typedef Maybe<T> return_t;
};

template <typename T> struct ToFactory;

template <> struct ToFactory<v8::Function> : ToFactoryBase<v8::Function> {
...
#define X(TYPE)                                                                \
  template <> struct ToFactory<v8::TYPE> : ToFactoryBase<v8::TYPE> {           \
    static inline return_t convert(v8::Local<v8::Value> val);                  \
  };
...
#define X(TYPE)                                                                \
  template <> struct ToFactory<TYPE> : ValueFactoryBase<TYPE> {                \
    static inline return_t convert(v8::Local<v8::Value> val);                  \
  };
