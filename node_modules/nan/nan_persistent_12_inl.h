...
public:
  ...
  template <typename S> inline void Reset(const v8::PersistentBase<S> &other) {
    v8::PersistentBase<T>::Reset(v8::Isolate::GetCurrent(), other);
  }
  ...
  inline void Copy(const Persistent<S, M2> &that) {
    ...
    (V8_MAJOR_VERSION > 4 ||                                                   \
     (V8_MAJOR_VERSION == 4 && defined(V8_MINOR_VERSION) &&                    \
      V8_MINOR_VERSION >= 3))
template <typename T> class Global : public v8::Global<T> {
public:
  ...
  inline void SetWeak(P *parameter,
                      typename WeakCallbackInfo<P>::Callback callback,
                      WeakCallbackType type) {
    reinterpret_cast<Persistent<T> *>(this)->SetWeak(parameter, callback, type);
  }
};
#else
template <typename T> class Global : public v8::UniquePersistent<T> {
public:
