...
template <typename T> class PersistentBase {
  ...
  template <typename U> friend v8::Local<U> New(const PersistentBase<U> &p);
  template <typename U, typename M>
  friend v8::Local<U> New(const Persistent<U, M> &p);
  template <typename U> friend v8::Local<U> New(const Global<U> &p);
...
public:
  ...
  template <typename S> inline void Reset(const v8::Local<S> &other) {
    TYPE_CHECK(T, S);
  ...
  template <typename S> inline void Reset(const PersistentBase<S> &other) {
    TYPE_CHECK(T, S);
  ...
  template <typename S> inline bool operator==(const v8::Local<S> &that) const {
...
public:
   ...
   * Move via assignment.
   */
  template <typename S> inline Global &operator=(Global<S> rhs) {
    TYPE_CHECK(T, S);
