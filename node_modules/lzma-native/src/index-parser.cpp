  ...
  case PARSE_INDEX_INITED:
    ...
    do {
      ...
      while (true) {
        ...
        ret = parse_indexes_read(info, internal->buf, LZMA_STREAM_HEADER_SIZE,
                                 internal->pos);
      ...
      // Decode the Index.
      ret =
          lzma_index_decoder(&internal->strm, &internal->this_index, memlimit);
      if (ret != LZMA_OK) {
        goto error;
      }
      ...
      do {
        ...
        ret = parse_indexes_read(info, internal->buf, internal->strm.avail_in,
                                 internal->pos);
      ...
      if (ret == LZMA_STREAM_END &&
          (internal->index_size != 0 || internal->strm.avail_in != 0)) {
        ret = LZMA_DATA_ERROR;
      }
    ...
    case PARSE_INDEX_READ_STREAM_HEADER:
      ...
      if (internal->combined_index != NULL) {
        ...
        // after this_index.
        ret = lzma_index_cat(internal->this_index, internal->combined_index,
                             info->allocator);
        if (ret != LZMA_OK) {
          goto error;
        }
...
int64_t IndexParser::readCallback(void *opaque, uint8_t *buf, size_t count,
  ...
  Local<Value> argv[2] = {Uint64ToNumberMaxNull(count),
                          Uint64ToNumberMaxNull(offset)};

  Local<Function> read_cb = Local<Function>::Cast(
      EmptyToUndefined(Nan::Get(handle(), NewString("read_cb"))));
...
Local<Object> IndexParser::getObject() const {
  ...
  Nan::Set(obj, NewString("streamPadding"),
           Uint64ToNumberMaxNull(info.stream_padding));
  Nan::Set(obj, NewString("memlimit"), Uint64ToNumberMaxNull(info.memlimit));
  Nan::Set(obj, NewString("streams"),
           Uint64ToNumberMaxNull(lzma_index_stream_count(info.index)));
  Nan::Set(obj, NewString("blocks"),
           Uint64ToNumberMaxNull(lzma_index_block_count(info.index)));
  Nan::Set(obj, NewString("fileSize"),
           Uint64ToNumberMaxNull(lzma_index_file_size(info.index)));
  Nan::Set(obj, NewString("uncompressedSize"),
           Uint64ToNumberMaxNull(lzma_index_uncompressed_size(info.index)));
  Nan::Set(obj, NewString("checks"),
           Uint64ToNumberMaxNull(lzma_index_checks(info.index)));
