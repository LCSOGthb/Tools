...
LZMAStream::LZMAStream()
    : bufsize(65536), shouldFinish(false), processedChunks(0),
...
#define LZMA_FETCH_SELF()                                                      \
  ...
  struct _MemScopeGuard {                                                      \
    ...
    ~_MemScopeGuard() { self->reportAdjustedExternalMemoryToV8(); }            \
                                                                               \
    LZMAStream *self;                                                          \
  };                                                                           \
...
#define POSSIBLY_UNLOCK_MX
  ...
  reportAdjustedExternalMemoryToV8();
  Local<Function> bufferHandler = Local<Function>::Cast(
      EmptyToUndefined(Nan::Get(handle(), NewString("bufferHandler"))));
...
#define CALL_BUFFER_HANDLER_WITH_ARGV                                          \
  ...
  while (outbufs.size() > 0) {
    ...
    Local<Value> argv[5] = {
        Nan::CopyBuffer(reinterpret_cast<const char *>(outbuf.data()),
                        outbuf.size())
            .ToLocalChecked(),
  ...
  if (processedChunks) {
    ...
    Local<Value> argv[5] = {Nan::Undefined(), Nan::New<Integer>(uint32_t(pc)),
                            Nan::Undefined(), in_, out_};
    CALL_BUFFER_HANDLER_WITH_ARGV
  }
...
NAN_METHOD(LZMAStream::New) {
  if (info.IsConstructCall()) {
    ...
    info.GetReturnValue().Set(
        Nan::NewInstance(Nan::New<Function>(constructor), 0, NULL)
            .ToLocalChecked());
  }
}

void LZMAStream::_failMissingSelf(
    const Nan::FunctionCallbackInfo<Value> &info) {
  Nan::ThrowTypeError(
      "LZMAStream methods need to be called on an LZMAStream object");
  info.GetReturnValue().SetUndefined();
}
...
NAN_METHOD(LZMAStream::MemlimitSet) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_memlimit_set(&self->_, NumberToUint64ClampNullMax(arg))));
}
...
NAN_METHOD(LZMAStream::RawEncoder) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_raw_encoder(&self->_, filters.array())));
}
...
NAN_METHOD(LZMAStream::RawDecoder) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_raw_decoder(&self->_, filters.array())));
}
...
NAN_METHOD(LZMAStream::FiltersUpdate) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_filters_update(&self->_, filters.array())));
}
...
NAN_METHOD(LZMAStream::EasyEncoder) {
  ...
  info.GetReturnValue().Set(lzmaRet(lzma_easy_encoder(
      &self->_, preset->Value(), (lzma_check)check->Value())));
}
...
NAN_METHOD(LZMAStream::StreamEncoder) {
  ...
  info.GetReturnValue().Set(lzmaRet(lzma_stream_encoder(
      &self->_, filters.array(), (lzma_check)check->Value())));
}
...
NAN_METHOD(LZMAStream::MTEncoder) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_stream_encoder_mt(&self->_, mt.opts())));
}
...
NAN_METHOD(LZMAStream::StreamDecoder) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_stream_decoder(&self->_, memlimit, flags->Value())));
}
...
NAN_METHOD(LZMAStream::AutoDecoder) {
  ...
  info.GetReturnValue().Set(
      lzmaRet(lzma_auto_decoder(&self->_, memlimit, flags->Value())));
}
