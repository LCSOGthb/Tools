...
NAN_METHOD(lzmaVersionString) {
  info.GetReturnValue().Set(
      Nan::New<String>(lzma_version_string()).ToLocalChecked());
}
...
NAN_METHOD(lzmaCheckIsSupported) {
  ...
  info.GetReturnValue().Set(
      (bool)lzma_check_is_supported((lzma_check)arg->Value()));
}
...
NAN_METHOD(lzmaCheckSize) {
  ...
  info.GetReturnValue().Set(
      Nan::New<Number>(lzma_check_size((lzma_check)arg->Value())));
}
...
NAN_METHOD(lzmaMfIsSupported) {
  ...
  info.GetReturnValue().Set(
      (bool)lzma_mf_is_supported((lzma_match_finder)arg->Value()));
}
...
NAN_METHOD(lzmaModeIsSupported) {
  ...
  info.GetReturnValue().Set(
      (bool)lzma_mode_is_supported((lzma_mode)arg->Value()));
}
...
NAN_METHOD(lzmaEasyEncoderMemusage) {
  ...
  info.GetReturnValue().Set(
      Uint64ToNumberMaxNull(lzma_easy_encoder_memusage(arg->Value())));
}
...
NAN_METHOD(lzmaEasyDecoderMemusage) {
  ...
  info.GetReturnValue().Set(
      Uint64ToNumberMaxNull(lzma_easy_decoder_memusage(arg->Value())));
}
...
NAN_METHOD(lzmaCRC32) {
  ...
  info.GetReturnValue().Set(
      Nan::New<Number>(lzma_crc32(data.data(), data.size(), arg->Value())));
}
...
NAN_METHOD(lzmaRawEncoderMemusage) {
  ...
  if (!filters.ok()) {
    Nan::ThrowTypeError(
        "rawEncoderMemusage requires filter array as arguments");
    info.GetReturnValue().SetUndefined();
    return;
  }

  info.GetReturnValue().Set(
      Uint64ToNumberMaxNull(lzma_raw_encoder_memusage(filters.array())));
}
...
NAN_METHOD(lzmaRawDecoderMemusage) {
  ...
  if (!filters.ok()) {
    Nan::ThrowTypeError(
        "rawDecoderMemusage requires filter array as arguments");
    info.GetReturnValue().SetUndefined();
    return;
  }

  info.GetReturnValue().Set(
      Uint64ToNumberMaxNull(lzma_raw_decoder_memusage(filters.array())));
}
