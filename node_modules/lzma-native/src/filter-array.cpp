...
FilterArray::FilterArray(Local<Array> arr) : ok_(false) {
  ...
  for (size_t i = 0; i < len; ++i) {
    Local<Object> entry =
        Local<Object>::Cast(EmptyToUndefined(Nan::Get(arr, i)));
    if (entry.IsEmpty() || entry->IsUndefined() || entry->IsNull() ||
        !Nan::Has(entry, id_).ToChecked()) {
      Nan::ThrowTypeError("Filter array needs object entries");
      return;
    }

    Local<String> id =
        Local<String>::Cast(EmptyToUndefined(Nan::Get(entry, id_)));
    Local<Object> opt =
        Local<Object>::Cast(EmptyToUndefined(Nan::Get(entry, options_)));
    ...
    case LZMA_FILTER_LZMA2:
      ...
      break;
    default:
      Nan::ThrowTypeError("LZMA wrapper library understands .options only for "
                          "DELTA and LZMA1, LZMA2 filters");
      return;
    }
