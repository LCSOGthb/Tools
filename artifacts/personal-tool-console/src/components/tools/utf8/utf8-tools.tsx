  ...
  const decoded = useMemo(() => {
    try {
      ...
      if (vals.length === 0 || vals.some((v) => Number.isNaN(v))) {
        return {
          value: "",
          error: "Enter hex bytes separated by spaces or commas",
        };
      }
    ...
    <div className="grid gap-4">
      ...
      <Field label={decodeMode ? "Hex bytes" : "Text"}>
        <TextArea
          ...
          placeholder={
            decodeMode
              ? "48 65 6c 6c 6f 20 2c 20 e4 b8 96 e7 95 8c 21"
              : "Hello, 世界!"
          }
        />
      </Field>
      {decodeMode ? (
        <>
          {decoded.error ? (
            ...
            <OutBox value={decoded.value} mono={false} />
          )}
        </>
      ) : (
        <>
    ...
    <div className="grid gap-4">
      <Field label="Text">
        <TextArea
          ...
          onChange={(e) => setText(e.target.value)}
          spellCheck={false}
          placeholder="A 😀 — try mixed scripts"
        />
      </Field>
      <p className="text-sm text-muted-foreground">
        {rows.length} codepoint{rows.length === 1 ? "" : "s"}
      </p>
      <div className="overflow-x-auto rounded-2xl border border-border bg-card/60 p-2">
...
function ValidatorPanel() {
  ...
  const result = useMemo(() => {
    const mode = /^(?:[0-9a-fA-F]{2}\s*)+$/.test(text.trim())
      ? "hex bytes"
      : "text";
    try {
    ...
    <div className="grid gap-4">
      ...
      {!result.valid && (
        <div className="rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-400">
          {first
            ? `${first.reason} at byte offset ${first.offset}`
    ...
    <div className="grid gap-4">
      <Field label="Text">
        <TextArea
          ...
          onChange={(e) => setText(e.target.value)}
          spellCheck={false}
          placeholder="Text to tag with a BOM"
        />
      </Field>
      <div className="flex flex-wrap items-center gap-2">
        <GhostButton type="button" onClick={() => setText(addBom(text))}>
          Add BOM
        </GhostButton>
        <GhostButton type="button" onClick={() => setText(removeBom(text))}>
          Strip BOM
        </GhostButton>
        <Chip
          color={
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        {hasBom && (
          <span className="text-xs text-muted-foreground/70">
            EF BB BF proves the BOM is there
          </span>
        )}
      </div>
    </div>
...
function EscapePanel() {
  ...
  const encoded = useMemo(
    () => escapeSequences(helper, mode as "unicode" | "hex" | "raw"),
    [helper, mode],
  );
  const decoded = useMemo(() => {
    try {
      return {
        value: unescapeSequences(reverse),
