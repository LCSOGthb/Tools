      ...
      <div className="grid gap-4 rounded-2xl border border-border bg-card/60 p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Field label="Mode">
            <SelectInput
              ...
              options={["Shuffle all", "Pick 1", "Pick N"]}
            />
          </Field>
          {mode === "Pick N" && (
            <Field label="N">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <PrimaryButton onClick={run}>
            {mode === "Pick 1" ? "Pick" : "Run"}
          </PrimaryButton>
          <Chip color="bg-ring/15 text-ring">
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-4 rounded-2xl border border-border bg-card/60 p-4 sm:grid-cols-3">
        ...
        <Field label="Seed">
          <NumInput value={seed} onChange={(e) => setSeed(e.target.value)} />
        </Field>
      ...
      {iban && (
        <div className="grid gap-4">
          <div className="grid gap-3 rounded-2xl border border-border bg-card/60 p-4">
            <Field label="Formatted IBAN">
              <OutBox value={iban} filename="iban.txt" />
            </Field>
            <Field label="Raw (no spaces)">
              <OutBox value={raw} filename="iban-raw.txt" />
            </Field>
          </div>
...
function BarcodeGen() {
  ...
  const [result, setResult] = useState<{ svg: string; width: number } | null>(
    null,
  );
...
function UnicodeRain() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      canvas.height = 300;
      drops = new Array(Math.max(1, Math.floor(canvas.width / fontSize)))
        .fill(0)
    ...
    const frame = (now: number) => {
      ...
      if (timer >= 50 / Math.max(0.2, speedRef.current)) {
        ...
        for (let i = 0; i < drops.length; i++) {
          const ch =
            charsetRef.current[
              Math.floor(Math.random() * charsetRef.current.length)
            ] || " ";
  ...
  return (
    <div className="grid gap-4">
      <div className="rounded-2xl border border-border bg-card/60 p-4">
        <canvas
          ref={canvasRef}
          className="w-full rounded-xl border border-border bg-[#030712]"
          style={{ height: 300 }}
        />
      </div>
      <div className="grid gap-4 rounded-2xl border border-border bg-card/60 p-4 sm:grid-cols-2">
        ...
        <div className="flex items-end">
          <GhostButton onClick={() => setPlaying((p) => !p)}>
            {playing ? "Pause" : "Play"}
          </GhostButton>
        </div>
      </div>
...
function SentencePicker() {
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-4 rounded-2xl border border-border bg-card/60 p-4">
        <Field
          label={`Paragraph — ${sentences.length} sentence${sentences.length === 1 ? "" : "s"} found`}
        >
          <TextArea
            value={text}
            onChange={(e) => setText(e.target.value)}
...
function FakeUserGen() {
  ...
  const generate = () => {
    ...
    const generated = Array.from({ length: n }, () => {
      ...
      const cc = rng
        ? String(Math.floor(rng() * 900) + 10)
        : String(Math.floor(Math.random() * 900) + 10);
  ...
  const csv = [
    "name,email,country,city,phone",
    ...users.map(
      (u) => `${u.name},${u.email},${u.country},${u.city},${u.phone}`,
    ),
  ].join("\n");
      ...
      <div className="grid gap-4 rounded-2xl border border-border bg-card/60 p-4 sm:grid-cols-2">
        ...
        <Field label="Name type">
          <SelectInput
            ...
            options={["Any", "Male", "Female", "Neutral"]}
          />
        </Field>
        <Field label="Seed (optional, for reproducible output)">
          <TextInput value={seed} onChange={(e) => setSeed(e.target.value)} />
        </Field>
        <div className="flex flex-wrap items-end gap-2">
          <PrimaryButton onClick={generate}>
            Generate {users.length ? "more" : "users"}
          </PrimaryButton>
          {users.length > 0 && (
            <>
...
export default function MiscTools({ tool }: ToolPageProps) {
  switch (tool.slug) {
    ...
    case "fake-iban-generator":
      return <FakeIbanGen />;
    case "barcode-generator":
      return <BarcodeGen />;
    case "unicode-rain":
      return <UnicodeRain />;
    case "online-sentence-picker":
