...
import { Field, NumInput, TextInput, OutBox, Chip } from "@/components/tools/shared/fields";
...
export default function RnShadow({ tool }: ToolPageProps) {
  ...
  const style = rnShadowStyle({ depth, color, iosOpacity, androidElevation, spread });
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Depth"><NumInput value={depth} min={1} max={20} onChange={(e) => setDepth(Number(e.target.value) || 1)} /></Field>
        <Field label="Color"><TextInput type="color" value={color} onChange={(e) => setColor(e.target.value)} /></Field>
        <Field label="iOS shadowOpacity (0–1)"><NumInput value={iosOpacity} min={0} max={1} step={0.05} onChange={(e) => setIosOpacity(Number(e.target.value) || 0)} /></Field>
        <Field label="Android elevation"><NumInput value={androidElevation} min={0} max={50} onChange={(e) => setAndroidElevation(Number(e.target.value) || 0)} /></Field>
        <Field label="Spread (optional)"><NumInput value={spread} min={0} max={50} onChange={(e) => setSpread(Number(e.target.value) || 0)} /></Field>
      </div>

      <OutBox value={style} mono filename="shadow-style.js" />

      <div className="rounded-2xl border border-border bg-card/60 p-4">
        <p className="mb-3 text-xs font-medium uppercase tracking-wide text-muted-foreground/70">Depth previews</p>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-5">
          {PRESETS.map((d) => (
            ...
            >
              <div className={`h-10 w-10 rounded-lg bg-card`} style={{ boxShadow: `0 ${d}px ${d * 2 + 2}px rgba(0,0,0,${0.1 + d * 0.06})` }} />
