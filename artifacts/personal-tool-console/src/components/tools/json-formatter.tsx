...
import { Field, TextArea, SelectInput, PrimaryButton, OutBox, Chip } from "@/components/tools/shared/fields";
...
export default function JsonFormatter({ tool }: ToolPageProps) {
  ...
  const run = () => {
    ...
    try {
      if (mode === "format") {
        setOutput(indent === "tab" ? JSON.stringify(JSON.parse(input), null, "\t") : formatJson(input, Number(indent)));
      } else if (mode === "minify") {
    ...
    <div className="grid gap-4">
        ...
        {(["format", "minify", "validate"] as Mode[]).map((m) => (
          <button
            ...
            className={`rounded-xl border px-4 py-2 text-sm capitalize transition ${
              mode === m ? "border-ring/60 bg-ring/10 text-foreground" : "border-border bg-background/60 text-muted-foreground hover:text-foreground"
            }`}
          >
            {m}
          </button>
      ...
      <div className="rounded-2xl border border-border bg-card/60 p-4">
        <Field label="Input JSON" hint="Paste JSON to transform or validate">
          <TextArea value={input} onChange={(e) => setInput(e.target.value)} spellCheck={false} placeholder='{"key": "value"}' />
        </Field>
        {mode === "format" && (
          <div className="mt-3">
            <SelectInput value={indent} onChange={(e) => setIndent(e.target.value)} options={["2", "4", "tab"]} />
          </div>
        )}
        <div className="mt-4">
      ...
      {mode === "validate" ? (
        <div className="rounded-2xl border border-border bg-card/60 p-4">
          {validation === null ? (
            <p className="text-sm text-muted-foreground">Run validation to check your JSON.</p>
          ) : validation.valid ? (
            ...
            <div className="text-red-400 text-sm">
              ...
              {typeof validation.line === "number" && <p className="mt-1">Line {validation.line}</p>}
              {validation.error && <p className="mt-1 break-words">{validation.error}</p>}
            </div>
          )}
        </div>
      ) : (
